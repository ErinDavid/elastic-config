...
public abstract class AbstractElasticConfig extends
		ConcurrentHashMap<String, String> implements ElasticConfig {
	 ...
	 * @return 此节点健值对
	 */
	protected abstract Optional<? extends Pair<String, String>> loadKey(
			String nodeName);
		...
		if (!children.isEmpty()) {
			...
			for (String child : children) {
				...
				if (keyValue.isPresent()) {
					configs.put(keyValue.get().getKey(), keyValue.get()
							.getValue());
				}
			}
	...
	public void reloadKey(final String nodeName) {
		try {
			...
			log.error("reload node path:{} error.",
					configProfile.getFullPath(nodeName), e);
	...
	private void removeRedundances(final Set<String> newkeyset) {
		...
		final Iterable<String> redundances = Iterables.filter(
				Sets.newHashSet(this.keySet()), new Predicate<String>() {
	...
	private void updateConfigGroup(final Map<String, String> newConfigs) {
		...
		for (Map.Entry<? extends String, ? extends String> entry : newConfigs
				.entrySet()) {
