...
public class ZookeeperElasticConfigGroup extends AbstractElasticConfig {
    ...
    public ZookeeperElasticConfigGroup(ZookeeperConfigProfile configProfile, String node) {
        super(configProfile);
    ...
    protected void configCenterInit() {
        ...
        this.configNodeStorage = new ConfigNodeStorage(RegistryCenterFactory.createCoordinatorRegistryCenter(
            configProfile.getServerlist(), configProfile.getNamespaces(), Optional.<String> absent()), configProfile);

    }

    @Override
    protected void checkConfigNodeIsExist() {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(configProfile.getNode()),
            "elastic config node must not empty");
        Preconditions.checkNotNull(configNodeStorage.isRootConfigNodeExisted(configProfile.getNode()),
    ...
    protected void addCacheData() {
        configNodeStorage.addDataCache(configProfile.getConcurrentRootNodePath());
    }
    ...
    protected List<String> getConfigNodeChildrenKey() {
        ...
        return configNodeStorage.getConfigNodeChildrenKeys(configProfile.getNode());
    }
     ...
     */
    @Override
    protected Optional<? extends Pair<String, String>> loadKey(final String nodeName) {
        ...
        return Optional.of(new ImmutablePair<String, String>(nodeName, configNodeStorage
            .getConfigNodeDataDirectly(nodeName)));
    }

    /**
     * 根据 配置加载模式确定节点是否包在在配置中
     * 
     * @param keyLoadingMode 配置项加载模式
     * @param nodeName 节点名称
     * @return 节点是否包在在配置中，true:配置中包含此节点，false:配置中排除此节点
     */
    private boolean checkKeyByLoadingMode(KeyLoadingMode keyLoadingMode, String nodeName) {
        boolean isinclude = true;
        Set<String> keysSpecified = configProfile.getKeysSpecified();

        if ((KeyLoadingMode.INCLUDE.equals(keyLoadingMode) && (!keysSpecified.contains(nodeName)))
            || (KeyLoadingMode.EXCLUDE.equals(keyLoadingMode) && keysSpecified.contains(nodeName))) {
            isinclude = false;
        }
    ...
    @Override
    public void close() {
        log.info("elastic config group {} begin close.", configNodeStorage.getConfigProfile().getNode());
